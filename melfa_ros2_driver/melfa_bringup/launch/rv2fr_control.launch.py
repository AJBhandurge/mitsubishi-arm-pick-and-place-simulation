...
def generate_launch_description():
    ...
    gz_launch_description_with_gui = IncludeLaunchDescription(
        ...
        launch_arguments={"gz_args":"/home/ayush/mitsubhishi_ws/src/melfa_ros2_driver/melfa_description/urdf/rv2fr/pick_place.sdf"}.items(),
    ...
    # ... (Keep all your existing imports and declared_arguments) ...

    # Update this specific section in your nodes list:
    gz_spawn_entity = Node(
        package="ros_gz_sim",
        executable="create",
        output="screen",
        arguments=[
            "-string", robot_description_content, # The robot description from Xacro
            "-name", "rv2fr",                # Unique name in Gazebo
            "-allow_renaming", "true",
            "x", "0",                           # X position (meters)
            "y", "0",                           # Y position (meters)
            "z", "0.1",                           # Z position (meters)
            "-Y", "0"                           # Yaw orientation (90 degrees in radians)
        ],
        condition=IfCondition(use_sim),
    )

    # ... (Keep the rest of your nodes list and return statement) ..
